...
def gpt3_completion(prompt, engine='text-davinci-003', temp=0.0, top_p=1.0, tokens=400, freq_pen=0.0, pres_pen=0.0, stop=['USER:', 'Kurie:']):
    max_retry = 5
    retry = 0
...
if __name__ == '__main__':
    ...
    while True:
        ...
        timestring = timestamp_to_datetime(timestamp)
        message = '%s: %s - %s' % ('Kurie', timestring, output)
        info = {'speaker': 'Kurie', 'time': timestamp, 'vector': vector, 'message': message, 'uuid': str(uuid4()), 'timestring': timestring}
        filename = 'log_%s_Kurie.json' % time()
        save_json('nexus/%s' % filename, info)
        #### print output
        print('\n\nKurie: %s' % output) 